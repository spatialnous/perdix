...
def getSelfIntersections(polyline):
    return [
        item for item, count in list(collections.Counter(polyline).items()) if count > 1
    ]  # points
...
def angle_3_points(p1, p2, p3):
    inter_vertex1 = math.hypot(
        abs(float(p2.x()) - float(p1.x())), abs(float(p2.y()) - float(p1.y()))
    )
    inter_vertex2 = math.hypot(
        abs(float(p2.x()) - float(p3.x())), abs(float(p2.y()) - float(p3.y()))
    )
    vertex1_2 = math.hypot(
        abs(float(p1.x()) - float(p3.x())), abs(float(p1.y()) - float(p3.y()))
    )
