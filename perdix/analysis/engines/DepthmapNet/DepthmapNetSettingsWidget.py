...
import os
from perdix.analysis.engines.DepthmapNet.DepthmapAdvancedDialog import (
...
Ui_DepthmapNetSettingsWidget, _ = loadUiType(
...
class DepthmapNetSettingsWidget(SettingsWidget, Ui_DepthmapNetSettingsWidget):
    ...
    def __init__(self, dock_widget):
        ...
        self.axialDepthmapSegmentRadio.clicked.connect(
            self.set_depthmap_segment_analysis
        )
        self.axialDepthmapSettingsButton.clicked.connect(
            self.show_axial_depthmap_advanced_settings
        )
        self.axialDepthmapRadiusText.editingFinished.connect(
            self.check_depthmap_input_text
        )
        self.axialDepthmapOutputText.editingFinished.connect(
            self.check_depthmap_input_text
        )
    ...
    def check_depthmap_input_text(self):
        if (
            self.axialDepthmapRadiusText.text() != ""
            and self.axialDepthmapOutputText.text() != ""
        ):
            ...
            self.set_axial_depthmap_calculate_tooltip(
                "Check if the radius values and output table name are correct."
            )
    ...
    def prepare_analysis_settings(self, analysis_layer, datastore):
        ...
        if datastore["type"] == 0:
            table_exists = shph.testShapeFileExists(
                datastore["path"], self.axial_analysis_settings["output"]
            )
    ...
    def get_analysis_summary(self):
        ...
        message += "\n   %s radius - %s" % (
            txt,
            self.axial_analysis_settings["rvalues"],
        )
        ...
        if (
            self.axial_analysis_settings["type"] in (1, 2)
