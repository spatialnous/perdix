...
class DepthmapEngine(AnalysisEngine):
    ...
    def prepare_axial_map(
        self, axial_layer, map_type, ref="", weight="", sep="\t", include_header=False
    ):
        try:
            ...
            if ref != "":
                if weight not in defaults:
                    for f in features:
                        ...
                        map_data += (
                            str(f.geometry().vertexAt(0).x())
                            + sep
                            + str(f.geometry().vertexAt(0).y())
                            + sep
                        )
                        map_data += (
                            str(f.geometry().vertexAt(1).x())
                            + sep
    ...
    def prepare_unlinks(
        ...
        if not lfh.fieldExists(unlinks_layer, "line1") or not lfh.fieldExists(
            unlinks_layer, "line2"
        ):
            raise AnalysisEngine.AnalysisEngineError(
                "Unlinks layer not ready for analysis: update and verify first."
            )
            return unlinks_data
        elif lfh.fieldHasNullValues(unlinks_layer, "line1") or lfh.fieldHasNullValues(
            unlinks_layer, "line2"
        ):
            raise AnalysisEngine.AnalysisEngineError(
                "Unlinks layer not ready for analysis: update and verify first."
        ...
        try:
            ...
            raise AnalysisEngine.AnalysisEngineError(
                "Exporting unlinks failed: " + str(e)
            )
        if unlinks_data != "":
    ...
    def process_analysis_result(settings, datastore, attributes, values):
        ...
        if settings["type"] in (1, 2) and settings["newnorm"] == 1:
            new_attributes, values = DepthmapEngine.calculate_normalised_segment(
                attributes,
                values,
                "Choice",
                "Node_Count",
    ...
    @staticmethod
    def calculate_normalised_segment(
        attributes,
        values,
        choice_col,
        node_count_col,
        ...
        for feat in values:
            ...
            for i, j in enumerate(choice):
                try:
                    val = math.log(float(feat[j]) + 1.0) / math.log(
                        float(feat[td[i]]) + 3.0
                    )
            ...
            for i, j in enumerate(td):
                try:
                    if i < len(nc):
                        ...
                        val = (float(feat[nc[i - len(nc)]]) ** 1.2) / (
                            float(feat[j]) + 2.0
                        )
                    calc_values.append(val)
                except Exception as e:
