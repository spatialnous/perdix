...
class UnlinksVerification(QThread):
    ...
    def __init__(
        self, parentThread, parentObject, settings, axial, axial_id, unlinks, id
    ):
    ...
    def run(self):
        ...
        if "spatialite" in datastore or "postgresql" in datastore:
            ...
            if "spatialite" in datastore:
                ...
                unlinkgeom = dbh.getPostgisGeometryColumn(
                    connection, unlinkinfo["schema"], unlinkname
                )
                axialinfo = dbh.getPostgisLayerInfo(self.axial_layer)
                axialgeom = dbh.getPostgisGeometryColumn(
                    connection, axialinfo["schema"], axialname
                )
            ...
            start_time = time.time()
            if "spatialite" in datastore:
                self.spatialiteTestUnlinks(
                    connection, unlinkname, unlinkgeom, axialname, axialgeom
                )
            else:
                self.postgisTestUnlinks(
                    connection,
                    unlinkinfo["schema"],
                    unlinkname,
                    unlinkgeom,
            ...
            lfh.addFields(
                self.unlinks_layer, ["line1", "line2"], [QVariant.Int, QVariant.Int]
            )
            # analyse the unlinks
            start_time = time.time()
    ...
    def spatialiteTestUnlinks(
        self, connection, unlinkname, unlinkgeom, axialname, axialgeom
    ):
        ...
        start_time = time.time()
        query = (
            """SELECT "%s", line1, line2 FROM "%s" WHERE NOT ST_IsSimple(%s) OR NOT ST_IsValid(%s)"""
        ...
        start_time = time.time()
        query = (
            'SELECT a."%s", a.line1, a.line2 FROM "%s" a, "%s" b WHERE a."%s" <> b."%s" AND ST_Equals(a."%s",b."%s") '
        ...
        start_time = time.time()
        query = (
            """SELECT "%s", line1, line2 FROM "%s" WHERE line1 IS NULL OR line2 IS NULL"""
            % (unlinkid, unlinkname)
        )
        header, data, error = dbh.executeSpatialiteQuery(connection, query)
        if data:
        ...
        # same line id
        start_time = time.time()
        query = """SELECT "%s", line1, line2 FROM "%s" WHERE line1 = line2""" % (
            unlinkid,
            unlinkname,
        )
        header, data, error = dbh.executeSpatialiteQuery(connection, query)
        ...
        start_time = time.time()
        query = (
            """CREATE TEMP TABLE "temp_unlinks_result" AS SELECT a."%s" unlinkid, b."%s" lineid FROM "%s" a, "%s" b WHERE %s(a."%s",b."%s"%s)"""
            % (
                unlinkid,
                axialid,
                unlinkname,
        ...
        start_time = time.time()
        query = (
            'SELECT "%s", line1, line2 FROM "%s" WHERE "%s" IN (SELECT unlinkid FROM (SELECT unlinkid, count(unlinkid) freq '
            "FROM temp_unlinks_result GROUP BY unlinkid) WHERE freq > 2)"
        ...
        query = (
            'SELECT "%s", line1, line2 FROM "%s" WHERE "%s" IN (SELECT unlinkid FROM (SELECT unlinkid, count(unlinkid) freq '
            "FROM temp_unlinks_result GROUP BY unlinkid) WHERE freq = 1)"
            % (unlinkid, unlinkname, unlinkid)
        )
        header, data, error = dbh.executeSpatialiteQuery(connection, query)
        if data:
        ...
        start_time = time.time()
        query = (
            """SELECT "%s", line1, line2 FROM "%s" WHERE "%s" NOT IN (SELECT unlinkid FROM temp_unlinks_result GROUP BY unlinkid)"""
            % (unlinkid, unlinkname, unlinkid)
        )
        header, data, error = dbh.executeSpatialiteQuery(connection, query)
        if data:
        ...
        start_time = time.time()
        query = (
            """SELECT b."%s", b.line1, b.line2 FROM temp_unlinks_result a, "%s" b WHERE a.unlinkid = b."%s" AND (a.lineid <> b.line1 AND a.lineid <> b.line2)"""
            % (unlinkid, unlinkname, unlinkid)
        )
        header, data, error = dbh.executeSpatialiteQuery(connection, query)
        if data:
    ...
    def postgisTestUnlinks(
        ...
        start_time = time.time()
        query = (
            """SELECT "%s", line1, line2 FROM "%s"."%s" WHERE NOT ST_IsSimple("%s") OR NOT ST_IsValid("%s")"""
        ...
        start_time = time.time()
        query = (
            """SELECT a."%s", a.line1, a.line2 FROM "%s"."%s" a, "%s".""%s" b WHERE a."%s" <> b."%s" AND ST_Equals(a."%s",b."%s")"""
            % (
                unlinkid,
                unlinkschema,
                unlinkname,
        ...
        start_time = time.time()
        query = (
            """SELECT "%s", line1, line2 FROM "%s"."%s" WHERE line1 IS NULL OR line2 IS NULL"""
        ...
        start_time = time.time()
        query = (
            """CREATE TEMP TABLE "temp_unlinks_result" AS SELECT a."%s" unlinkid, b."%s" lineid FROM "%s"."%s" a, "%s"."%s" b WHERE %s(a."%s",b."%s"%s)"""
            % (
                unlinkid,
                axialid,
                unlinkschema,
        ...
        start_time = time.time()
        query = (
            'SELECT "%s", line1, line2 FROM "%s"."%s" WHERE "%s" IN (SELECT unlinkid FROM (SELECT unlinkid, count(*) freq '
        ...
        start_time = time.time()
        query = (
            """SELECT "%s", line1, line2 FROM "%s"."%s" WHERE "%s" NOT IN (SELECT unlinkid FROM temp_unlinks_result GROUP BY unlinkid)"""
            % (unlinkid, unlinkschema, unlinkname, unlinkid)
        )
        header, data, error = dbh.executePostgisQuery(connection, query)
        if data:
        ...
        start_time = time.time()
        query = (
            """SELECT b."%s", b.line1, b.line2 FROM temp_unlinks_result a, "%s"."%s" b WHERE a.unlinkid = b."%s" AND (a.lineid <> b.line1 AND a.lineid <> b.line2)"""
            % (unlinkid, unlinkschema, unlinkname, unlinkid)
        )
        header, data, error = dbh.executePostgisQuery(connection, query)
        if data:
    ...
    def qgisTestUnlinks(self):
        ...
        if self.user_id == "":
            ...
            features = self.unlinks_layer.getFeatures(
                QgsFeatureRequest().setSubsetOfAttributes([field, line1, line2])
            )
        # run unlinks tests
...
class UnlinksIdUpdate(QThread):
    ...
    def __init__(
        self, parentThread, parentObject, unlinks, id, axial, axial_id, threshold
    ):
        QThread.__init__(self, parentThread)
        self.parent = parentObject
        self.running = False
    ...
    def spatialiteUpdateIDs(self, connection, unlinktype):
        ...
        # add line id columns
        dbh.addSpatialiteColumns(
            connection, unlinkname, ["line1", "line2"], [QVariant.Int, QVariant.Int]
        )
        ...
        if self.user_id == "":
            ...
            if not lfh.isValidIdField(self.unlinks_layer, unlinkid):
                ...
                header, data, error = dbh.executeSpatialiteQuery(
                    connection, query, commit=True
                )
        if self.axial_id == "":
            axialid = "ROWID"
        else:
        ...
        query = (
            "UPDATE %s SET line1 = (SELECT c.line FROM (SELECT b.%s unlink, a.%s line FROM "
        ...
        query = (
            "UPDATE %s SET line2 = (SELECT c.line FROM (SELECT b.%s unlink, a.%s line FROM "
    ...
    def postgisUpdateIDs(self, connection, unlinktype):
        ...
        # update line id columns
        query = (
            'UPDATE "%s"."%s" ul SET line1 = ax.line FROM (SELECT b."%s" unlink, a."%s" line FROM '
        ...
        self.verificationProgress.emit(66)
        query = (
            'UPDATE "%s"."%s" ul SET line2 = ax.line FROM (SELECT b."%s" unlink, a."%s" line FROM '
