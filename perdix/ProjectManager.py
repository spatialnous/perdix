...
class ProjectDialog(QDialog, Ui_ProjectDialog):
    ...
    def __init__(self, iface, proj_settings, settings):
        ...
        self.dataTypeCombo.addItems(
            ["Shape files folder", "Personal geodatabase", "PostGIS database"]
        )
    ...
    def loadDatastoreList(self):
        ...
        if self.datastore_type == 0:
            ...
            if len(con_settings) > 0:
                ...
                self.datastores["idx"] = self.datastores["name"].index(
                    dbh.getPostgisSelectedConnection()
                )
                path = []
        ...
        if self.datastore_type and data_type and (self.datastore_type == data_type):
            ...
            if self.datastore_type == 0 and os.path.exists(
                self.proj_settings["datastore/path"]
            ):
                self.appendDatastoreList(
                    self.proj_settings["datastore/name"],
                    self.proj_settings["datastore/path"],
                )
            # select the datastore if in the list
            try:
                self.datastores["idx"] = self.datastores["path"].index(
                    self.proj_settings["datastore/path"]
                )
            except Exception as e:
                print(
    ...
    def loadSchemaList(self, name):
        if name:
            ...
            self.schemaLabel.setDisabled(False)
            try:
                idx = self.datastores["schema"].index(
                    self.proj_settings["datastore/schema"]
                )
    ...
    def openDatastore(self):
        ...
        if self.datastore_type == 0:
            path = QtWidgets.QFileDialog.getExistingDirectory(
                self, "Select shape files folder", lastDir
            )
            if path.strip() != "":
                path = str(path)
            ...
            path = QtWidgets.QFileDialog.getOpenFileName(
                self, "Open Spatialite data base", lastDir, "Spatialite (*.sqlite *.db)"
            )
            if path.strip() != "":
                path = str(path)
                name = os.path.basename(path)
                ...
                if self.datastores and name in self.datastores["name"]:
                    self.iface.messageBar().pushMessage(
                        "Error",
                        "A database already exists with the same name.",
                        level=1,
                        duration=5,
                    )
                    append = False
