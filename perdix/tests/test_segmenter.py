...
class TestNetworkSegmenter(unittest.TestCase):
    ...
    def test_merge_nodes(self):
        ...
        if errors:
            ...
            break_point_feats = [
                my_segmentor.copy_feat(
                    my_segmentor.break_f, QgsGeometry.fromPointXY(p_fid[0]), p_fid[1]
                )
            ...
            ids2 = [
                i
                for i in range(
                    max(ids1) + 1, max(ids1) + 1 + len(my_segmentor.invalid_unlinks)
                )
            ]
            invalid_unlink_point_feats = [
                my_segmentor.copy_feat(
                    my_segmentor.invalid_unlink_f,
            ...
            stubs_point_feats = [
                my_segmentor.copy_feat(
                    my_segmentor.stub_f, QgsGeometry.fromPointXY(p_fid2[0]), p_fid2[1]
                )
        ...
        for segmented_feat in segmented_feats:
            ...
            for expected_segment in expected_segments:
                x1eq = (
                    abs(segmented_line[0].x() - expected_segment[0].x())
                    < error_tolerance
                )
                y1eq = (
                    abs(segmented_line[0].y() - expected_segment[0].y())
                    < error_tolerance
                )
                x2eq = (
                    abs(segmented_line[1].x() - expected_segment[1].x())
                    < error_tolerance
                )
                y2eq = (
                    abs(segmented_line[1].y() - expected_segment[1].y())
                    < error_tolerance
                )
        ...
        self.assertAlmostEqual(
            stubs_point_feats[0].geometry().asPoint().x(), 535012.560936, places=3
        )
        self.assertAlmostEqual(
            stubs_point_feats[0].geometry().asPoint().y(), 186126.546602, places=3
        )
