...
class AttributeSymbology(QObject):
        ...
        if mode < 3:
            ...
            for i in range(0, len(range_steps) - 1):
                ...
                if symbol:
                    ...
                    this_range = QgsRendererRange(
                        range_steps[i], range_steps[i + 1], symbol, label
                    )
    ...
    def getColourRamp(self, colour_type, invert):
        ...
        if colour_type == 2:  # grey scale
            if (invert and canvas.value() >= 80) or (
                not invert and canvas.value() < 80
            ):
                ramp = QgsGradientColorRamp(
                    QColor(0, 0, 0, 255), QColor(248, 248, 248, 255), False
                )
            else:
                ramp = QgsGradientColorRamp(
                    QColor(248, 248, 248, 255), QColor(0, 0, 0, 255), False
                )
        if colour_type == 3:  # monochrome
            if canvas.value() < 80:
                ramp = QgsGradientColorRamp(
                    QColor(255, 255, 255, 255), QColor(255, 255, 255, 255), False
                )
            else:
                ramp = QgsGradientColorRamp(
                    QColor(0, 0, 0, 255), QColor(0, 0, 0, 255), False
                )
