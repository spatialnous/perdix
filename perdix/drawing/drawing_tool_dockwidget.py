...
class DrawingToolDockWidget(QtWidgets.QDockWidget, Ui_DrawingToolDockWidget):
    ...
    def setAxialSnapping(self):
        ...
        if self.settings[0] != "":
            ...
            snapConfig.setMode(QgsSnappingConfig.SnappingMode.AdvancedConfiguration)
            layerSnapConfig = QgsSnappingConfig.IndividualLayerSettings(
                False,
                QgsSnappingConfig.SnappingType.Vertex,
                self.settings[2],
                QgsTolerance.UnitType.LayerUnits,
            )
            ...
            self.iface.messageBar().pushMessage(
                "Network layer not specified!", Qgis.MessageLevel.Critical, duration=5
            )
            self.axial_mode = False
        return
